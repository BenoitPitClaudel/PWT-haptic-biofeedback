...
%%%%%%%%%%%%%%%%%%%%%%% Controls UI Box Param %%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 lowlimitbox_hr = uicontrol('parent', hp, 'style', 'edit','string', '50', ...
 ...
 lowlimitbox_br = uicontrol('parent', hp, 'style', 'edit','string', '5', ...
 ...
 log_time_button = uicontrol('position', [40, 40, 150, 40],...
     'string', 'Log Time', 'CallBack', @logTime);
 participant_id = uicontrol('style', 'edit', 'string', 'dummy_id', ...
     'position', [10, 90, 100, 30], 'fontsize',10);
 uicontrol('style', 'text', 'backgroundcolor', bgcolor,...
     'position', [17, 120, 90, 20], 'fontsize', 10,...
     'HorizontalAlignment', 'left', 'string', 'Participant ID');
 experiment_state = uicontrol('style', 'edit', 'string', 'dummy_state', ...
     'position', [120, 90, 100, 30], 'fontsize',10);
 uicontrol('style', 'text', 'backgroundcolor', bgcolor,...
     'position', [135, 120, 90, 20], 'fontsize', 10,...
     'HorizontalAlignment', 'left', 'string', 'Experiment');
%  uicontrol('parent', hp, 'style', 'text', 'backgroundcolor', bgcolor,...
...
%%%%%%%%%%%%%%%%%%%%%%%%%% connect functions %%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    function connect_bh(obj, ~)
        if (bh_connected == 0)
            ...
            rd_conn = serial(radar_port, 'InputBufferSize', 1472, 'baudrate', 9600);
            ...
            try
                ...
                fclose(rd_conn);
                delete(rd_conn);
                set (hStatus, 'String', 'Error opening radar data stream.');
                set(hConnectMenu, 'enable', 'on');
                return;
            end
            try
...
%%%%%%%%%%%%%%%%%%%%%%%%%%% data processing funcitons %%%%%%%%%%%%%%%%
    ...
    function save_radar_raw()
        radar_data = zeros([32 3]);
        while rd_conn.bytesAvailable < 736
        end
        read = fscanf(rd_conn, '%s', min(rd_conn.bytesAvailable, 736));
        str_read = string(read);
        lines = strsplit(str_read, ';');
        for i = 1:32
           temp = strsplit(lines(i), ',');
           radar_data(i, 1) = str2double(temp(1));
           radar_data(i, 2) = str2double(temp(2));
           radar_data(i, 3) = str2double(temp(3));
        end
        if ~exist(fullfile(participant_id.String), 'dir')
            mkdir(participant_id.String)
        end
        dlmwrite(strcat(participant_id.String, '/', ...
            participant_id.String, '_', experiment_state.String, ...
            '_radar_raw.csv'),radar_data,'delimiter',',','-append'); 
    end
...
% rr data
    ...
    function logTime(~, ~)
        time_logs = [time_logs cputime-start_time];
    end
